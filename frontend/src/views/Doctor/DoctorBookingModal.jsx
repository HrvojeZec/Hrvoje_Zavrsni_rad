...
export function DoctorBookingModal({ opened, doctor, onConfirm, onClose }) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await fetch(`${appConfig.apiUrl}/api/reservations/doctors/${doctor.id}`, {
                    headers: {
    ...
    return (
        <Modal title="Rezerviraj termin" centered opened={opened} onClose={onClose}>
            <LoadingOverlay visible={confirmLoading || reservationsLoading} />

            <Group position="center">
                <Avatar src={doctor.avatar} size={48} radius={48} />
                <div>
